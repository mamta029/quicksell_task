  ...
  return (
        ...
        <div className="list-container">
            <div className="list-header">
                ...
                props.ticketDetails.map(ticket => {
                    ...
                    // Check if this user has already been rendered
                    if (userName === props.listTitle && !renderedUsers.has(userName)) {
                        renderedUsers.add(userName); // Mark this user as rendered
                        return (
                            <div className="card-profile" key={userName}>
                                <div className="card-profile-initial">
                                    {userName[0]}{userName[1]}
                                </div>
                                <div className={ticket.userObj.available 
                                    ? "card-profile-initial-available card-profile-initial-initial-available-true" 
                    ...
                    return null; // Skip rendering for this user if already rendered
                })
            }</>,
                            'priority' : <>{
                                {
                        ...
                        {
                            {
                                ...
                                'user' : <span style={{ marginLeft: '5px' }}>{props.listTitle}</span>
                            }[props.groupValue]
                        }
                    </div>
    ...
    props.ticketDetails.map(ticket => {
        if (ticket.status === props.listTitle || ticket.priority === props.listTitle || ticket.userObj.name === props.listTitle) {
            cardCount++;
            return <Card key={ticket.id} cardDetails={ticket} groupValue={props.groupValue} /> // Pass groupValue here
        }
        return null;
    }, cardCount = 0)
}
